...
void EffectManager::Create(DX::DeviceResources * deviceResources, const wchar_t* name,int count)
{
	...
	DirectX::CreateWICTextureFromFile(deviceResources->GetD3DDevice(), L"Resources\\Textures\\gingham.png", nullptr, m_texture2.GetAddressOf());

	// コンパイルされたシェーダファイルを読み込み
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shaders/ParticleVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shaders/ParticleGS.cso");
	BinaryFile fadeIn_PS = BinaryFile::LoadFile(L"Resources/Shaders/FadeIn.cso");
	BinaryFile fadeOut_PS = BinaryFile::LoadFile(L"Resources/Shaders/FadeOut.cso");
	...
	// ピクセルシェーダ作成
	if (FAILED(device->CreatePixelShader(fadeIn_PS.GetData(), fadeIn_PS.GetSize(), NULL, m_fadeInPS.ReleaseAndGetAddressOf())))
	{// エラー
		MessageBox(0, L"CreatePixelShader Failed.", NULL, MB_OK);
		return;
	}
	// ピクセルシェーダ作成
	if (FAILED(device->CreatePixelShader(fadeOut_PS.GetData(), fadeOut_PS.GetSize(), NULL, m_fadeOutPS.ReleaseAndGetAddressOf())))
	{// エラー
		MessageBox(0, L"CreatePixelShader Failed.", NULL, MB_OK);
		return;
	}
...
void EffectManager::Draw(DirectX::SimpleMath::Matrix world, DirectX::SimpleMath::Matrix view, DirectX::SimpleMath::Matrix proj,int type)
{
	auto context = m_deviceResources->GetD3DDeviceContext();
	...
	//定数バッファをシェーダに渡す（とりあえずVSはいらないのでコメントアウト）
	...
	{
	case FADE_IN:
		context->PSSetShader(m_fadeInPS.Get(), nullptr, 0);
		break;
	case FADE_OUT:
		context->PSSetShader(m_fadeOutPS.Get(), nullptr, 0);
		break;
	default:
		assert("存在しないピクセルシェーダーファイルが呼び出されました");
		break;
	}
